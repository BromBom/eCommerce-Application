...
import { getCartItems, setCartItems, parseRequestUrl, rerender } from '../../../utils/localstorage';
import { CartItem } from '../../../types/types';

export const addToCart = (item: CartItem, forceUpdate = false): void => {
  ...
  render: async (): Promise<string> => {
    ...
    if (request.id) {
      try {
        ...
        const product = response.body.results.find((p: ProductProjection) => p.id === request.id);
        if (product) {
          const price =
            product.masterVariant.prices && product.masterVariant.prices.length > 0
              ? product.masterVariant.prices[0].value.centAmount / 100
              : 0;
          const quantityInStock = 10;
          const image =
